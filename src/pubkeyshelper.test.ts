import { PubkeysHelper } from './pubkeyshelper'
import { PubkeysHelperTestServer } from './pubkeyshelper-test-server'

const expectedKeysResponse = {
  '26c018b233fe2eef47fedbbdd9398170fc9b29d8@RS256': {
      publicKey:
      '-----BEGIN PUBLIC KEY-----\\nuQIDAQAB\n-----END PUBLIC KEY-----'
  },
  'ba4ded7f5a92429f233561a36ff613ed38762c3d@RS256': {
      publicKey:
      '-----BEGIN PUBLIC KEY-----\\n9QIDAQAB\n-----END PUBLIC KEY-----'
  },
  '9c37bf73343adb93920a7ae80260b0e57684551e@RS256': {
      publicKey:
      '-----BEGIN PUBLIC KEY-----\\nfQIDAQAB\n-----END PUBLIC KEY-----'
  }
}

describe('PubkeysHelper', () => {
  const server = new PubkeysHelperTestServer()
  let pubkeysHelper: PubkeysHelper
  let baseUrl: string
  beforeAll(async () => {
    await server.start()
    pubkeysHelper = new PubkeysHelper()
    baseUrl = `http://localhost:${server.listenPort}`
  })
  afterEach(() => {
    server.reset()
  })
  afterAll(async () => {
    await server.stop()
  })

  it('fetchJwkKeys with options', async () => {
    let pubkeys = await pubkeysHelper.fetchJwkKeys(`${baseUrl}/publickeys`, {
      expiresSkew: 25200
    })
    return expect(pubkeys).toEqual({
      '26c018b233fe2eef47fedbbdd9398170fc9b29d8@RS256': {
        publicKey:
          '-----BEGIN PUBLIC KEY-----\\nuQIDAQAB\n-----END PUBLIC KEY-----',
        expiresSkew: 25200
      },
      'ba4ded7f5a92429f233561a36ff613ed38762c3d@RS256': {
        publicKey:
          '-----BEGIN PUBLIC KEY-----\\n9QIDAQAB\n-----END PUBLIC KEY-----',
        expiresSkew: 25200
      },
      '9c37bf73343adb93920a7ae80260b0e57684551e@RS256': {
        publicKey:
          '-----BEGIN PUBLIC KEY-----\\nfQIDAQAB\n-----END PUBLIC KEY-----',
        expiresSkew: 25200
      }
    })
  })

  it('fetchJwkKeys', async () => {
    const output = await pubkeysHelper.fetchJwkKeys(`${baseUrl}/publickeys`)
    expect(output).toEqual(expectedKeysResponse)
  })
  
  it('fetchJwkKeys with empty keys', async () => {
    await expect(pubkeysHelper.fetchJwkKeys(`${baseUrl}/emptykeys`)).rejects.toThrow(
      new Error(`No keys found in response from ${baseUrl}/emptykeys`)
    )
  })
  it('fetchJwkKeys with empty json', async () => {
    await expect(pubkeysHelper.fetchJwkKeys(`${baseUrl}/emptyjson`)).rejects.toThrow(
      new Error(
        `Response from ${baseUrl}/emptyjson not in expected format: Missing array property keys`
      )
    )
  })
})
